...
if (index == "saramaki")
    ...
    for (i in 1:num.ind)
      { if (i== step * stepwidth) {
        ...
        F=0
         ...
         temp = (Ki[i] - 1.0)
         if (temp == 0) temp = 1.0
         if (i == 1) Cw = F / (Ki[i] * temp) else Cw = c(Cw, F / (Ki[i] * temp))
      }
 } else
 {
  PSbinary = matrix(0, num.ind, num.ind)
 ...
 close(pb)
CW = sum(Cw)/num.ind;
if ((CW + O) != 0) CwS = (CW - O)/(CW + O)

# Calculate the binary matrix
 PSbinary = matrix(0, num.ind, num.ind)
 PSbinary[PS >= O] = 1
Ris = list(Omean = O, E = 1 - O, PS = PS, PSbinary = PSbinary, Ejack = Ejack, VarE = vare, CW = CW, CwS = CwS, Cw= Cw, index = index)
cat("\n Araujo's E ", 1 - O)
